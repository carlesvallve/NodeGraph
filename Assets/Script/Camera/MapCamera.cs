...
public class MapCamera : MonoBehaviour {
	...
	public void SetPosition (float x, float y) {
		...
		currentCamera.transform.position = new Vector3(x, y, currentCamera.transform.position.z);
		ConstrainToBounds();
	}
	...
	public void SetPositionAndZoom (float x, float y, float zoom) {
		...
		Vector3 pos = new Vector3(x, y, currentCamera.transform.position.z);
	...
	private Vector3 ConstrainToBounds (Vector3 pos, float atZoom = 0, bool updatePosition = true) {
		if (atZoom == 0) { 
			atZoom = currentCamera.orthographicSize; 
		}
